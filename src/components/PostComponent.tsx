  ...
  return (
    <div className={cardStyle}>
      <Link href={`/posts/${post?.slug?.current}`}>
        <h2 className={`${font.className} text-2xl text-[#ce9251] dark:text-slate-300`}>{post?.title}</h2>
        <p className={`${dateFont.className} my-2 `}>{new Date(post?.publishedAt).toDateString()}</p>
